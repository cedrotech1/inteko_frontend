...
const LandingPage = () => {
  ...
  const officerMenu = [
    ...
    { name: 'post', icon: <BiBuilding />, to: '/post' }, 
    { name: 'Settings', icon: <BiCog />, to: '/settings' },
  ];
  ...
  const getMenu = () => {
    if (obj.role === 'admin') {
      return adminMenu;
    } else if (obj.role === 'province_leader' || obj.role === 'district_leader' || obj.role === 'sector_leader' || obj.role === 'cell_leader' || obj.role === 'village_leader') {
      return officerMenu;
    } else {
      return [];
    }
    ...
    <div style={{ border: '0px solid green', backgroundColor: 'white', color: 'green', marginTop: '1cm' }} 
      ...
      <div className="text-center mt-3">
        <Link to="/logout" className="btn btn-sm" 
              style={{ border: '1px solid lightblue', backgroundColor: 'lightblue', color: 'black', marginTop: '', width: '80%' }}>
          Logout
        </Link>
      </div>
    </div>
